...
def get_group_with_projects(session, server, group_name, repo_matcher):
    group = session.get("{0}/api/v4/groups/{1}".format(server, group_name))
    global req_group
    req_group += 1
...
def is_merge_request_in_project_group(merge_request, group, repo_matcher, repo_excluder):
    return project_id_in_group_and_allowed(merge_request["target_project_id"], group, repo_matcher, repo_excluder, "MR[id={0},{1}]".format(merge_request["id"], encode_text(merge_request["title"])))

def is_issue_in_project_group(issue, group, repo_matcher, repo_excluder):
    return project_id_in_group_and_allowed(issue["project_id"], group, repo_matcher, repo_excluder, "Issue[id={0},{1}]".format(issue["id"], encode_text(issue["title"])))

def get_project(project_id):
    if project_id not in projectCache:
        project_request = session.get("{0}/api/v4/projects/{1}".format(gitlab_server, project_id))
        project_request.raise_for_status()
        projectCache[project_id]=project_request.json()
    else:
        projectCache.get(project_id)
    return projectCache[project_id]

def project_id_in_group_and_allowed(project_id, group, repo_matcher, repo_excluder, source_object_debug):
    project = get_project(project_id)
    project_is_child_of_org = True if "/{0}/".format(group["path"]) in project["web_url"] else False
    ...
    if project_is_child_of_org and repo_name_matches and repo_name_excluded == False:
        if is_debug: print "Accepted: {0} from project: {1}/{2}".format(source_object_debug, project["id"], encode_text(project["name"]))
        return True
    else:
        if is_debug: print "Excluded: {0} due to project name: {1}/{2}".format(source_object_debug, project["id"], encode_text(project["name"]))
        return False
...
if is_debug: start = round(time.time() * 1000)
...
if is_debug: print "Script took {0} seconds to run".format(((round(time.time() * 1000))-start)/1000)
